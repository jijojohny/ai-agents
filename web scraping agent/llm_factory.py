...
Optional packages: only the provider you use must be installed (see requirements.txt notes).
"""
...
# Default model IDs per provider (override with model_name or env WEB_SCRAPER_MODEL)
DEFAULT_MODELS = {
    "openai": "gpt-4o-mini",
...
def _resolve_model(provider: str, model_name: Optional[str]) -> str:
    ...
    env_key = os.getenv("WEB_SCRAPER_MODEL")
    if env_key:
...
def build_chat_model(
    ...
    Args:
        provider: One of: openai, anthropic, google, gemini (alias of google), vertex
        model_name: Model id; defaults per provider or WEB_SCRAPER_MODEL
