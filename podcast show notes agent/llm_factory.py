...
def _model(provider: str, model_name: Optional[str]) -> str:
    if model_name and str(model_name).strip():
        return str(model_name).strip()
    e = os.getenv("PODCAST_SHOW_NOTES_AGENT_MODEL")
    return e.strip() if e else _DEFAULT.get(provider, _DEFAULT["openai"])


def build_chat_model(provider: str, model_name: Optional[str] = None, temperature: float = 0.5) -> Any:
    p = (provider or "openai").strip().lower()
    if p == "gemini":
        p = "google"
    m = _model(p, model_name)
    if p == "openai":
